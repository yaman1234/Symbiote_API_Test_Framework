...
async function loginWithOtp(client, { email, password, otp }) {
  ...
  if (!loginRes.ok()) {
    return { ok: false, step: 'login', status: loginRes.status(), body: loginBody };
  }
  ...
  if (!sendRes.ok()) {
    return { ok: false, step: 'send-otp', status: sendRes.status(), body: sendBody };
  }
  ...
  if (!verifyRes.ok()) {
    return { ok: false, step: 'verify-otp', status: verifyRes.status(), body: verifyBody };
  }
